...
function sendQuery() {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            ...
            let errorMessage = "Desculpe, algo deu errado. Tente novamente mais tarde! Rate LIMITE execed";

            // If there's an error response from the backend, show it
            if (xhr.responseJSON && xhr.responseJSON.error) {
                errorMessage = xhr.responseJSON.error;
            }

            // Display the error message
            $('#chat').append('<li class="chat bot-message"><p>' + errorMessage + '</p></li>');
            $('#chat').scrollTop($('#chat')[0].scrollHeight);
        }
    });
}
